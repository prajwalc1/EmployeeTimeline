...
import { initializeDatabase, closeDatabase, isDatabaseConnected, getDatabase } from "@db";
...
// Updated getDataDirectory function for better Windows support
function getDataDirectory(): string {
  ...
  try {
    if (process.platform === 'win32') {
      ...
      const dataDir = path.join(appDataPath, 'SchwarzenbergTech', 'TimeManagement');
    ...
    const unixDataDir = path.join(os.homedir(), '.local', 'share', 'SchwarzenbergTech', 'TimeManagement');
    if (!fs.existsSync(unixDataDir)) {
      fs.mkdirSync(unixDataDir, { recursive: true });
    }
    return unixDataDir;
  } catch (error) {
